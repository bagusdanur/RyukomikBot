...
def _clean_id(value: str) -> str:
    value = str(value or "").strip("/").removeprefix("manga/").removeprefix("series/").removeprefix("comics/")
    return value


def _clean_chapter(value: str) -> str:
    value = str(value or "").strip("/").removeprefix("reader/en/").removeprefix("chapter/").split("/")[-1]
    if value.isdigit():
...
def _ext(url: str) -> str:
    ...
    return extension.lstrip(".") if extension in {".jpg", ".jpeg", ".png", ".webp", ".gif", ".avif"} else "jpg"


class MgekoDownloader:
    """Downloader for the Ryukomik Mgeko API."""

    def __init__(self):
        self.api_url = MGEKO_API.rstrip("/")
    ...
    async def get_chapter_images(self, manga_id: str, chapter_id: str) -> List[str]:
        clean_manga = _clean_id(manga_id)
        clean_chap = _clean_chapter(chapter_id)
        ...
        if requested_number and needs_resolution:
            ...
            if matched and matched.get("id"):
                clean_chap = _clean_chapter(matched["id"])
    ...
    async def download_chapter(self, manga_id: str, chapter_id: str, save_dir: str) -> Optional[str]:
        ...
        clean_manga = _clean_id(manga_id)
        clean_chap = _clean_chapter(chapter_id)
        target = os.path.join(save_dir, "mgeko", f"{clean_manga}_{clean_chap}")
        async with _session() as session:
...
async def search_mgeko(query: str) -> List[Dict[str, Any]]:
    return await mgeko_downloader.search_manga(query)
