...
def _clean_id(value: str) -> str:
    value = str(value or "").strip("/").removeprefix("series/").removeprefix("manga/").removeprefix("comics/")
    return value
...
class KaganeDownloader:
    """Downloader for the Ryukomik Kagane API."""

    def __init__(self):
        self.api_url = KAGANE_API.rstrip("/")
    ...
    async def download_chapter(self, manga_id: str, chapter_id: str, save_dir: str) -> Optional[str]:
        ...
        clean_manga = _clean_id(manga_id)
        clean_chap = _clean_chapter(chapter_id)
        target = os.path.join(save_dir, "kagane", f"{clean_manga}_{clean_chap}")
        async with _session() as session:
...
async def search_kagane(query: str) -> List[Dict[str, Any]]:
    return await kagane_downloader.search_manga(query)
