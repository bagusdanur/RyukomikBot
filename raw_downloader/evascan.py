...
class EvaScanDownloader:
    """Downloader for the Ryukomik EvaScan API."""

    def __init__(self):
        self.api_url = EVASCAN_API.rstrip("/")

    async def search_manga(self, query: str) -> List[Dict[str, Any]]:
        async with _create_session() as session:
            payload = await get_json(session, f"{self.api_url}/search", source="evascan", stage="search", params={"q": query}, timeout=4, validator=lambda item: isinstance(item.get("data"), list))
        if not payload:
            return []
        return [{"id": _clean_id(item.get("slug", item.get("id", ""))), "title": item.get("title", "Unknown"), "status": item.get("status", item.get("type_genre", "N/A")), "chapter_count": item.get("update", item.get("chapter_count", "N/A")), "rating": item.get("rating", "N/A"), "image": item.get("image", ""), "source": "evascan"} for item in payload.get("data", []) if item.get("slug") or item.get("id")]

    async def get_manga_info(self, manga_id: str) -> Optional[Dict[str, Any]]:
        clean_id = _clean_id(manga_id)
        async with _create_session() as session:
            payload = await get_json(session, f"{self.api_url}/detail/{clean_id}", source="evascan", stage=f"detail:{clean_id}", timeout=4, validator=lambda item: bool((item.get("data") or item).get("chapters")))
...
async def search_evascan(query: str) -> List[Dict[str, Any]]:
    return await evascan_downloader.search_manga(query)
