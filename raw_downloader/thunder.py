...
class ThunderDownloader:
    """Downloader for the Ryukomik Thunder API."""

    def __init__(self):
        self.api_url = THUNDER_API.rstrip("/")

    async def search_manga(self, query: str) -> List[Dict[str, Any]]:
        async with _create_session() as session:
            payload = await get_json(session, f"{self.api_url}/search", source="thunder", stage="search", params={"q": query}, timeout=4, validator=lambda item: isinstance(item.get("data"), list))
        if not payload:
            return []
        return [{"id": _clean_id(item.get("slug", item.get("id", ""))), "title": item.get("title", "Unknown"), "status": item.get("status", item.get("type_genre", "N/A")), "chapter_count": item.get("update", item.get("chapter_count", "N/A")), "rating": item.get("rating", "N/A"), "image": item.get("image", ""), "source": "thunder"} for item in payload.get("data", []) if item.get("slug") or item.get("id")]

    async def get_manga_info(self, manga_id: str) -> Optional[Dict[str, Any]]:
        clean_id = _clean_id(manga_id)
        async with _create_session() as session:
            payload = await get_json(session, f"{self.api_url}/detail/{clean_id}", source="thunder", stage=f"detail:{clean_id}", timeout=4, validator=lambda item: bool((item.get("data") or item).get("chapters")))
    ...
    async def get_chapter_list(self, manga_id: str) -> List[Dict[str, Any]]:
        ...
        if not info:
            return []
        clean_id = _clean_id(manga_id)
        return [{"id": _clean_chapter(chapter.get("slug", chapter.get("link", chapter.get("title", "")))), "title": chapter.get("title", "Unknown Chapter"), "date": chapter.get("date", chapter.get("time", "")), "manga_id": clean_id, "source": "thunder"} for chapter in info.get("chapters", []) if chapter.get("slug") or chapter.get("link") or chapter.get("title")]
...
async def search_thunder(query: str) -> List[Dict[str, Any]]:
    return await thunder_downloader.search_manga(query)
