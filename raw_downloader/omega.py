...
def _clean_id(value: str) -> str:
    ...
    return value.removeprefix("series/").removeprefix("manga/")
...
class OmegaDownloader:
    """Downloader matching the Ryukomik Omega API schema."""

    def __init__(self):
        self.api_url = OMEGA_API.rstrip("/")
    ...
    async def get_manga_info(self, manga_id: str) -> Optional[Dict[str, Any]]:
        ...
        async with _create_session() as session:
            payload = await get_json(
                ...
                source="omega",
                stage=f"detail:{clean_id}",
                timeout=4,
                validator=lambda item: bool((item.get("data") or {}).get("chapters")),
            )
    ...
    async def get_chapter_images(self, manga_id: str, chapter_id: str) -> List[str]:
        ...
        async with _create_session() as session:
            payload = await get_json(
                ...
                source="omega",
                stage=f"chapter:{clean_manga}:{clean_chapter}",
                timeout=10,
...
async def search_omega(query: str) -> List[Dict[str, Any]]:
    return await omega_downloader.search_manga(query)
