...
import { api, type PerformanceBonus, type PerformanceBonusSettings } from "../api";
...
async function load() {
  ...
  try { [items.value, settings.value] = await Promise.all([api.performanceBonuses(period.value), api.performanceBonusSettings()]); }
...
async function approve(item: PerformanceBonus) {
  saving.value = true;
  try { await api.approvePerformanceBonus(item.id); success.value = `Bonus ${item.staff_name || item.staff_id} disetujui dan akan masuk invoice berikutnya.`; detail.value = null; await load(); }
...
<template>
  <div class="bonus-page">
    ...
    <section class="bonus-hero">
      <div><span class="kicker">MONTHLY REWARD</span><h3>Bonus Performa Staff</h3><p>Evaluasi privat berbasis kualitas, deadline, dan konsistensi. Bonus hanya aktif setelah persetujuan admin.</p></div>
    ...
    <section class="bonus-summary">
      ...
      <article><span>Batas per staff</span><strong>{{ money(settings.max_amount) }}</strong><small>per bulan</small></article>
    </section>
    <section class="bonus-list">
      <div class="section-title"><div><h4>Kandidat {{ period }}</h4><p>Klik card untuk memeriksa bukti perhitungan.</p></div><span>{{ filtered.length }} staff</span></div>
      ...
      <button v-for="item in filtered" :key="item.id" class="bonus-card" @click="detail=item">
        <div class="staff"><div class="avatar">{{ (item.staff_name || item.staff_id).slice(0,1).toUpperCase() }}</div><div><strong>{{ item.staff_name || `Staff ${item.staff_id}` }}</strong><span>{{ item.approved_chapters }} chapter · {{ money(item.eligible_earnings) }}</span></div></div>
    ...
    <div v-if="detail" class="bonus-modal" @click.self="detail=null"><section class="bonus-sheet">
      <header><div><span class="kicker">BUKTI PERHITUNGAN</span><h3>{{ detail.staff_name || detail.staff_id }}</h3><p>Periode {{ detail.period }}</p></div><button @click="detail=null">×</button></header>
