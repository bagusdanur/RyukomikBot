    ...
    >
      <form class="modal-card" @submit.prevent="createTask">
        ...
        <div class="form-grid">
          ...
          ><label class="wide">Paket Download RAW<select v-model="task.raw_pack_mode"><option value="normal">Normal — satu file per halaman</option><option value="webp_hd">WebP HD — kualitas tinggi, ukuran lebih kecil</option><option value="merge_16000">Gabung WebP HD — maksimal sekitar 16.000 px</option></select><small>{{ task.raw_pack_mode === 'merge_16000' ? 'Beberapa halaman digabung vertikal tanpa mengecilkan dimensi, lalu disimpan sebagai WebP kualitas 95 agar ukuran lebih ringan.' : task.raw_pack_mode === 'webp_hd' ? 'Setiap halaman dikonversi ke WebP kualitas 95. Dimensi mengikuti Mode RAW yang dipilih.' : 'Setiap halaman RAW tetap dalam format sumber.' }}</small></label
