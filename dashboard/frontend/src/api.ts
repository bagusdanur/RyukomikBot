export type User = { id: string; username: string; avatar: string | null; role: 'admin' | 'staff' }
export type Assignment = {
  id: number; manga: string; chapter: string; staff_id: string | null; role: string
  final_rate: number; status: string; deadline_at: string | null; assigned_at: string
  staff_name?: string; staff_avatar?: string | null
}
export type Staff = { id: string; staff_id: string; username: string; avatar: string | null; task_count: number; active_count: number; approved_amount: number; paid_amount: number }
export type Recap = { staff_id: string; staff_name: string; staff_avatar: string | null; chapter_count: number; total_amount: number; pending_amount: number; paid_amount: number }
export type Invoice = { id: number; invoice_number: string; staff_id: string; staff_name: string; staff_avatar: string | null; period: string; chapter_count: number; total_amount: number; status: string; issued_at: string; paid_at: string | null }
...
export type Submission = { id:number;assignment_id:number;staff_id:string;original_name:string;size_bytes:number;uploaded_at:string;manga:string;chapter:string;role:string }
...
const liveApi = {
  ...
  createInvoice: (staff_id: string, period: string) => request('/api/invoices', { method: 'POST', body: JSON.stringify({ staff_id, period }) }),
